  ...
  const handleTerbitkan = () => {
    ...
    const hasil = pesertaList.map((p, index) => {
      ...
      return {
        ...p,
        kodeUnik: kode,
        link: `http://localhost:3000/verifikasi/${kode}`
      };
    });

    console.log('Sertifikat yang diterbitkan:', hasil);
    alert(`Berhasil menerbitkan ${hasil.length} sertifikat!\nCek console untuk detailnya.`);
  };

  return (
    <div className="min-h-screen bg-gray-50 py-10">
      <div className="max-w-5xl mx-auto px-6">
        <h1 className="text-3xl font-bold mb-2">Terbitkan Sertifikat (Bulk)</h1>
        <p className="text-gray-600 mb-8">Import Excel peserta lalu terbitkan sertifikat sekaligus</p>

        <div className="bg-white p-8 rounded-3xl shadow mb-8">
          <h2 className="font-semibold mb-4">1. Isi Informasi Webinar</h2>
            ...
            <div>
              ...
              <div className="max-h-60 overflow-auto border rounded-2xl">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {pesertaList.slice(0, 5).map((row, i) => (
              ...
              {pesertaList.length > 5 && (
                <p className="text-xs text-gray-500 mt-2">...dan {pesertaList.length - 5} peserta lainnya</p>
              )}
            </div>
          )}
        </div>
        ...
        <button 
          ...
          className="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white py-4 rounded-2xl font-semibold text-lg"
        >
